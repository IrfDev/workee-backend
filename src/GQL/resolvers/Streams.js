...
const resolvers = {
  Stream: {
    ...
    feedlyItems: async (stream, _, ctx) => {
      ...
      const feedlyItems = await stream.feedlyStreamsid.map(
        async (collectionId) => {
          const streamObject = await ctx.streams.usecases.getStreamsFromFeedly(
            collectionId,
          );
          console.log('StreamsObject', streamObject);
          return streamObject;
        },
      );
